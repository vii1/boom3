...
//   BOOM 2 Engine                                                      //
//                                                                      //
//   Clip.cpp - class Clip implementation                             //
//                                                                      //
//   by Ivaylo Beltchev                                                 //
...
namespace boom {
	...
	void Clip::cut( coord2d x1c, coord2d x2c )
	{
		...
		if( xmin < x1c ) {
			...
			if( x < x1c ) {
				coord2d yu, yd;
				traps = new Trap( tr->x2, yu = splitab( tr->y11, tr->y21, x1c - x, tr->x2 - x1c ),
		...
		if( x2c < xmax ) {
			...
			if( x2c < t->x2 ) {
				coord2d yu, yd;
				end = new Trap( t->x2, yu = splitab( t->y11, t->y21, x2c - x, t->x2 - x2c ),
	...
	void Clip::addbeg( Monotone *poly )
	{
		if( beg ) {
			*lbeg = poly->traps;
			poly->traps = beg;
	...
	void Clip::addend( Monotone *poly )
	{
		if( end ) {
			Trap *t;
	...
	void Clip::clip( coord2d x1, coord2d x2, coord2d y11, coord2d y12, coord2d y21, coord2d y22, Monotone *poly )
	{
		poly->xmin = x1;
		poly->xmax = x2;
		poly->traps = NULL;
