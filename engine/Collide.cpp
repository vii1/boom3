...
namespace boom {
	...
	bool Map::move( coord3d dx, coord3d dy, coord3d dz, angle ax, angle ay, angle az )
	{
		bool r = true;
		coord3d x, y, z;
		gdx = dx;
		...
		for( int i = 0; i < COLIDE_TRYS; i++ ) {
			...
			for( int li = 0; li < clinenum; li++ ) {
				Line *l = clines[li];
				coord3d x1 = m_boom.verts[l->v1].x;
				coord3d y1 = m_boom.verts[l->v1].y;
				coord3d x2 = m_boom.verts[l->v2].x;
				coord3d y2 = m_boom.verts[l->v2].y;
				coord3d lx1 = (y2 - y1);
				coord3d ly1 = -(x2 - x1);
				if( li > 0 ) {
	...
	bool Map::collision( coord3d x, coord3d y, coord3d *z )
	{
		coord3d zc = 10000;
		coord3d zf = -10000;
		Cluster *c0 = m_boom.cur_cluster;
		cur_cluster = NULL;
		clinenum = 0;
		int b1, b2 = 0;
		Cluster *c = clusters;
	...
	bool Sector::inside( coord3d x, coord3d y )
	{
		int n = 0;
		int i;
	...
	bool Sector::inside( coord3d x, coord3d y, coord3d z )
	{
		if( x<minx || x>maxx || y<miny || y>maxy ) return false;
	...
	int Sector::collision( coord3d x, coord3d y, coord3d z )
	{
		int f = 0;
		int i;
		Line *l;
	...
	bool Line::collide( coord3d z, coord3d zf, coord3d zc, double t )
	{
		Wall **w = walls;
