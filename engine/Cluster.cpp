...
//   BOOM 2 Engine                                                      //
//                                                                      //
//   Cluster.cpp - class Cluster implementation                       //
//                                                                      //
//   by Ivaylo Beltchev                                                 //
...
namespace boom {
	...
	bool Cluster::load()
	{
		...
		int n = rdlong();
		if( n ) {
			CONSTRUCT_AND_LOAD_ARRAY( sectors, Sector, n );
		}
		sectorsnum = n;
		return true;
	}
	...
	Sector *Cluster::inside( coord3d x, coord3d y )
	{
		Sector *s = sectors;
		for( int i = 0; i < sectorsnum; i++, NEXTSECTOR( s ) ) {
	...
	static bool before( Cont *c1, Cont *c2 )
	{
	...
	void Cluster::sort_contours( int n )
	{
		int i, j, k;
	...
	void Cluster::reset_contours()
	{
		if( cont ) free( cont );
		cont = NULL;
		options &= ~kOptionVisited;
	}
...
#define MAX_CONT 40
	...
	void Cluster::draw()
	{
		...
		for( conti++; conti < contn; conti++ ) {
			Sector *s = cont[conti].s;
			int n = cont[conti].n + 1;
			for( Line *l = cont[conti].l1; --n; l = l->next_in_contour() )
				l->draw( s );
		}
		conti = oldconti;
	}
