...
// https://www.gamedev.net/articles/programming/graphics/xtreme-debugging-r590/
...
void mono_print::printf(const char* format, ...)
{
	char    temp_string[256];   // holds working copy of string
	va_list arglist;
	va_start( arglist, format );
	vsnprintf( temp_string, sizeof( temp_string ), format, arglist );
	va_end( arglist );
	this->print( temp_string );
}
...
void mono_print::draw( char* string, int x, int y, int style )
{
	...
	for( index = 0; index < length; ++index ) {   // OJO
...
void mono_print::scroll( int num_lines )
{
	...
	while( num_lines-- > 0 ) {
		...
		memset( (void*)( mono_video + 24 * MONO_BYTES_PER_LINE / 2 ), 32, MONO_BYTES_PER_LINE );
	}   // end while
...
void mono_print::clear()
{
	...
	// clear the display
	memset( (void*)mono_video, 32, 25 * MONO_BYTES_PER_LINE );
  ...
  int                            field_width )   // size of ouput field, for blank padding
{
	...
	if( num_watches >= MONO_MAX_WATCHES ) return ( -1 );
...
void mono_print::update_watches( void )
{
	...
	for( index = 0; index < num_watches; index++ ) {
		...
		switch( watch[index].type ) {
			...
			case MONO_WATCH_DOUBLE: {
				sprintf( temp_string, "%s=%f", watch[index].name, *( (double*)( watch[index].data ) ) );
			} break;
			...
			case MONO_WATCH_TVECTOR: {
				Tvector* vec = (Tvector*)watch[index].data;
				sprintf( temp_string, "%s=x:%f, y:%f, z:%f", watch[index].name, vec->x, vec->y, vec->z );
			} break;

			// case MONO_WATCH_FIX16: {
			// 	int n = sprintf( temp_string, "%s=", watch[index].name );
			// 	if( n > 0 ) {
			// 		fix16_to_str( *(fix16_t*)watch[index].data, temp_string + n, 5 );
			// 	}
			// } break;
		...
		strncat( temp_string, " ", watch[index].field_width - ( strlen( temp_string ) - strlen( watch[index].name ) - 1 ) );
